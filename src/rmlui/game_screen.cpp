...
namespace client::ui
{
    ...
    void GameScreen::ChooseColor( int cardId )
    {
        ...
        {
            Rml::ElementPtr colorBut =
                    Rml::Factory::InstanceElement( window, "button", "button", Rml::XMLAttributes() );
    ...
    void GameScreen::CreatePlayersCards()
    {
        ...
        for ( std::size_t i = 0; i < clState.Players.size(); i++ )
        {
            ...
            if ( info.IsLocal )
            {
                ...
                for ( std::size_t j = 0; j < info.Hand.size(); j++ )
                {
    ...
    void GameScreen::CreateCard( const Card& card, int index, Rml::Element* container )
    {
        ...
        if ( playable )
            cardW->Animate( "margin-bottom", Rml::Property( 2.0f, Rml::Unit::VH ), 0.15,
                            {Rml::Tween::Quadratic, Rml::Tween::Out} );
