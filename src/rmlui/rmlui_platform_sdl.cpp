...
namespace client::ui
{
    ...
    {
        const Rml::Vector2f position = Rml::Vector2f{ev.tfinger.x, ev.tfinger.y} * Rml::Vector2f{
                                           context->GetDimensions()
                                       };
        return {Rml::Touch{finger_id, position}};
    }
    ...
    bool RmlSDL::InputEventHandler( Rml::Context* context, SDL_Window* window, SDL_Event& ev )
    {
        ...
        switch ( ev.type )
        {
            case event_mouse_motion:
            {
                ...
                result = context->ProcessMouseMove( static_cast<int>(ev.motion.x * pixel_density),
            ...
            {
                result = context->ProcessMouseWheel( -ev.wheel.y, GetKeyModifierState() );
                break;
            }
            case event_key_down:
            {
