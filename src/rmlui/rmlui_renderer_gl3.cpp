...
void main() {
    ...
    namespace Gfx
    {
        static const char* const program_uniform_names[ (size_t)UniformId::Count ] = {
            ...
            "_texelOffset", "_texCoordMin", "_texCoordMax", "_texMask", "_weights[0]", "_func", "_p", "_v",
            "_stop_colors[0]", "_stop_positions[0]",
        ...
        static bool CreateProgram( GLuint& out_program, Uniforms& inout_uniform_map, ProgramId program_id,
        ...
        static bool CreateShaders( ProgramData& data )
        {
            ...
            RMLUI_ASSERT(
                std::all_of(data.programs.begin(), data.programs.end(), [](auto&& value) { return value == 0; }) );
            ...
            {
                if ( !CreateProgram( data.programs[ def.id ], data.uniforms, def.id,
                                     data.vert_shaders[ def.vert_shader ], data.frag_shaders[ def.frag_shader ] ) )
    ...
    void RenderInterface_GL3::EndFrame()
    {
        ...
        glColorMask( glstate_backup.color_writemask[ 0 ], glstate_backup.color_writemask[ 1 ],
                     glstate_backup.color_writemask[ 2 ],
                                                                      ...
                                                                      Rml::Span<const int> indices )
    {
        ...
        glBufferData( GL_ARRAY_BUFFER, sizeof( Rml::Vertex ) * vertices.size(), (const void*)vertices.data(),
                      draw_usage );
        ...
        glBufferData( GL_ELEMENT_ARRAY_BUFFER, sizeof( int ) * indices.size(), (const void*)indices.data(),
                      draw_usage );
...
#pragma pack()
    ...
    Rml::TextureHandle RenderInterface_GL3::LoadTexture( Rml::Vector2i& texture_dimensions, const Rml::String& source )
    {
        ...
        for ( long y = 0; y < header.height; y++ )
        {
            ...
            long write_index = top_to_bottom_order
                                   ? ( y * header.width * 4 )
    ...
    {
        RMLUI_ASSERT(
            source_data.data() && source_data.size() == size_t(source_dimensions.x * source_dimensions.y * 4) );
    ...
    static void SetTexCoordLimits( GLint tex_coord_min_location, GLint tex_coord_max_location,
                                   Rml::Rectanglei rectangle_flipped,
                                   Rml::Vector2i framebuffer_size )
    {
        ...
        const Rml::Vector2f min = ( Rml::Vector2f( rectangle_flipped.p0 ) + Rml::Vector2f( 0.5f ) ) / Rml::Vector2f(
                                      framebuffer_size );
        const Rml::Vector2f max = ( Rml::Vector2f( rectangle_flipped.p1 ) - Rml::Vector2f( 0.5f ) ) / Rml::Vector2f(
                                      framebuffer_size );
    ...
    {
        RMLUI_ASSERT(
            &source_destination != &temp && source_destination.width == temp.width && source_destination.height == temp.
            height );
        ...
        SetTexCoordLimits( GetUniformLocation( UniformId::TexCoordMin ), GetUniformLocation( UniformId::TexCoordMax ),
                           scissor,
                                                                  ...
                                                                  const Rml::Dictionary& parameters )
    {
        ...
        if ( name == "linear-gradient" )
        {
            ...
            shader.gradient_function = ( repeating
                                             ? ShaderGradientFunction::RepeatingLinear
                                             : ShaderGradientFunction::Linear );
        ...
        } else if ( name == "radial-gradient" )
        {
            ...
            const bool repeating = Rml::Get( parameters, "repeating", false );
            shader.gradient_function = ( repeating
                                             ? ShaderGradientFunction::RepeatingRadial
        ...
        } else if ( name == "conic-gradient" )
        {
            ...
            const bool repeating = Rml::Get( parameters, "repeating", false );
            shader.gradient_function = ( repeating
                                             ? ShaderGradientFunction::RepeatingConic
        ...
        for ( const Rml::CompiledFilterHandle filter_handle: filter_handles )
        {
            ...
            switch ( type )
            {
                ...
                case FilterType::DropShadow:
                {
                    ...
                    const Rml::Vector2f uv_offset = filter.offset / Rml::Vector2f(
                ...
                case FilterType::ColorMatrix:
                {
                    ...
                    const GLint uniform_location = program_data->uniforms.Get(
                        ProgramId::ColorMatrix, UniformId::ColorMatrix );
                    constexpr bool transpose = std::is_same<decltype(filter.color_matrix),
                        Rml::RowMajorMatrix4f>::value;
    ...
    bool RmlGL3::Initialize( Rml::String* out_message )
    {
        ...
        if ( out_message )
            *out_message = Rml::CreateString( "Loaded OpenGL %d.%d.", GLAD_VERSION_MAJOR( gl_version ),
